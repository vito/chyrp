<form class="settings" action="<?php url("update_user"); ?>" method="post" accept-charset="utf-8">
	...
	<p>
		...
		<input class="text" type="text" name="login" value="<?php echo fix($user->login, "html"); ?>" id="login" />
	</p>
	<p>
...
<?php while ($group = $get_groups->fetchObject()): ?>
			<option value="<?php echo $group->id; ?>"<?php selected($group->id, $user->group_id); ?>><?php echo $group->name; ?></option>
<?php endwhile; ?>
		</select>
	</p>
	...
	<p>
		...
		<input class="text" type="text" name="full_name" value="<?php echo fix($user->full_name, "html"); ?>" id="full_name" tabindex="1" />
	</p>
	<p>
		<label for="email"><?php echo __("E-Mail"); ?></label>
		<input class="text" type="text" name="email" value="<?php echo fix($user->email, "html"); ?>" id="email" tabindex="1" />
	</p>
	<p>
		<label for="website"><?php echo __("Website"); ?></label>
		<input class="text" type="text" name="website" value="<?php echo fix($user->website, "html"); ?>" id="website" tabindex="1" />
	</p>
