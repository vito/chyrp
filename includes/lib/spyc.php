<?php
  /**
  * A few things have been commented out since Chyrp doesn't need them, such as
  * [these], {these}, and comments (# foo)
  */
  /**
     ...
     */
     public function load($input) {
      ...
      foreach ($yaml as $linenum => $line) {
        ...
        } elseif (substr($ifchk,0,3) != '---') { # } elseif ($ifchk{0} != '#' && substr($ifchk,0,3) != '---') {
     ...
     private function _toType($value) {
      if (preg_match('/^("(.*)"|\'(.*)\')/',$value,$matches)) {
       ...
       $value = preg_replace('/\\\\"/','"',$value);
        ...
        $value = trim($value); # $value = trim(preg_replace('/#(.+)$/','',$value));
      }
