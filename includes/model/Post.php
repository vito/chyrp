<?php
	...
	class Post {
		 ...
		 * 
		 * Parameters:
		 * 	$yaml - The data to insert, generated by Spyc::YAMLDump.
		 ...
		 * 	<update>
		 */
		static function add($yaml, $clean = "", $url = "") {
			...
			# Append any options to the YAML dump
			if (!empty($options)) {
				$load_yaml = Spyc::YAMLLoad($yaml);
				foreach ($options as $key => $val)
					$load_yaml[$key] = $val;
				$yaml = Spyc::YAMLDump($load_yaml);
			}
		 ...
		 * 	<add>
		 */
		public function update($yaml, $pinned = null, $status = null, $slug = null, $timestamp = null) {
			if (!isset($this->id)) return;
			
			fallback($pinned, !empty($_POST['pinned']));
			fallback($status, (isset($_POST['draft'])) ? "draft" : ((!empty($_POST['status'])) ? $_POST['status'] : $this->info("status", $post_id)));
			fallback($slug, (!empty($_POST['slug'])) ? $_POST['slug'] : $this->info("feather", $post_id).".".$post_id);
			fallback($timestamp, (!empty($_POST['created_at'])) ? when("Y-m-d H:i:s", $_POST['created_at']) : $this->info("created_at", $post_id));
		...
		/**
		 * Function: edit_link
		 * Outputs an edit link for the given post ID, if the <User.can> edit_post.
		 * 
		 * Parameters:
		 * 	$post_id - The post ID for the link.
		 ...
		 * Outputs a delete link for the given post ID, if the <User.can> delete_post.
		 * 
		 * Parameters:
		 * 	$post_id - The post ID for the link.
		 * 	$text - The text to show for the link.
		 ...
		 * Generates an acceptable Title from a given post ID's excerpt.
		 * 
		 * Parameters:
		 * 	$post_id - The post ID to generate the Title from.
		 * 
		 * Returns:
		 ...
		 * 
		 * Parameters:
		 * 	$yaml - The XML to load.
		 */
		private function parse($filter = false) {
			global $post;
			
			$parse = Spyc::YAMLLoad($this->yaml);
